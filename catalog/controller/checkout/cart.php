...
class ControllerCheckoutCart extends Controller {
	...
	public function index() {
		...
		if ($this->cart->hasProducts() && !(!$this->cart->hasStock() && !$this->config->get('config_stock_checkout')) && !$noRedirect) {
    	...
    	if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
      		...
      		foreach ($products as $product) {
        		...
        		foreach ($product['option'] as $option) {
					...
					$option_data[] = array(
						...
						'value' => (utf8_strlen($value) > 200 ? utf8_substr($value, 0, 200) . '..' : $value)
					);
        		}
			...
			$this->data['continue'] = $this->url->link('product/category','path=59');
...
$this->data = array_merge( $this->data , $this->language->load('checkout/cart'));
	...
	public function quick_order() {
		...
		if (isset($this->request->post['name']) && isset($this->request->post['phone']) && isset($this->request->get['product_id'])) {
			$json['error'] = array();
			...
			if (empty($this->request->post['phone'])) {
				$json['error']['phone'] = "Пожалуйста, введите свой номер телефона";
			}

			if (empty($json['error'])) {
				$product_name = $this->db->query("SELECT name FROM product_description WHERE product_id = ". (int)$this->request->get['product_id'] ." AND language_id = '" . (int)$this->config->get('config_language_id') . "'")->row['name'];
				
				$html = "<h3>Вам пришла заявка на быстрый заказ</h3>
				
				Товар: <a href=". $this->url->link("product/product","product_id" . $this->request->get['product_id']) .">$product_name</a><br/>
				Колличество: <b>". $this->request->post['qty'] ."</b><br/>
				Имя покупателя: <b>". $this->request->post['name'] ."</b><br/>
				Телефон покупателя: <b>". $this->request->post['phone'] ."</b><br/>
				";
				...
				$mail->setSubject($this->config->get('config_name') . " - Вам пришла заявка на быстрый заказ");
				$mail->setHtml($html);
				$mail->send();

				$json['success'] = "Ваш заказ принят! Скоро с Вами свяжется наш менеджер. Спасибо.";
			}
		}
