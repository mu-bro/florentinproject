...
$this->data = array_merge( $this->data , $this->language->load('product/product'));
		...
		if ($product_info) {
			...
			if ($product_info['quantity'] <= 0) {
				...
				$this->data['stock'] = "eсть";
			}

			$this->load->model('tool/image');

			if ($product_info['image']) {
				$this->data['popup'] = $this->model_tool_image->resize($product_info['image'], 700, 700);
			} else {
				$this->data['popup'] = $this->model_tool_image->resize('no_image.jpg', 700, 700);
			}
			...
			foreach ($results as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], 200, 230);
				} else {
					$image = $this->model_tool_image->resize('no_image.jpg', 200, 230);
				}
