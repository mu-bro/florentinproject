...
$this->data = array_merge( $this->data , $this->language->load('account/order'));
...
$this->data = array_merge( $this->data , $this->language->load('account/order'));
		...
		if ($order_info) {
			...
			if ($order_info['shipping_type'] == 1) {
				$this->data['payment_address'] .= ",<br/>Доставка: на адрес плательщика,<br/>" . "Дата доставки: " . $order_info['payment_delivery_date'];
			}
//	p($order_info);
			...
			if ($order_info['shipping_address_format']) {
				...
				$format = '{firstname},' . "\n" . '{address_1},' . "\n" . 'Дата доставки: {delivery_time},' . "\n" . 'Звонить получателю перед доставкой: {need2call}';
			}
			...
			if ($order_info['shipping_type'] == 0) {
				...
				$replace = array(
					...
					'delivery_time' => $order_info['shipping_delivery_date'],
					'shipping_type'      => ($order_info['shipping_type'] == 1) ? "Звонить" : "Сделать сюрприз",
				);

				$this->data['shipping_address'] = str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($find, $replace, $format))));

				if (($order_info['shipping_type'] == 1)) {
					$this->data['shipping_address'] .= ",<br/>" . "Телефон: " . $order_info['shipping_telephone'];
				}
