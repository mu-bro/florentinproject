	...
	public function index() {
		...
		if (!$json) {
			...
			foreach ($this->cart->getProducts() as $product) {
				...
				if ($product['image']) {
					$image = $this->model_tool_image->resize($product['image'], 60, 86);
				} else {
					$image = '';
				}
	...
	public function createorder(){
		...
		if (!$json) {
			...
			if ($this->customer->isLogged()) {
				...
				$data['payment_delivery_date'] = ($this->session->data['shipping_type'] == 1) ? $this->session->data['payment']['delivery_time'] : "";
				...
				if ( $data['shipping_type'] == 1 ) {
					$data['payment_delivery_date'] = $this->session->data['payment']['delivery_time'];
				} else {
					$data['payment_delivery_date'] = "";
				}
			}
			...
			if ($this->cart->hasShipping() && $this->session->data['shipping_type'] == 0) {

				$shipping_address = $this->session->data['guest']['shipping'];

				if ( $data['shipping_type'] == 0 ) {
					$data['shipping_delivery_date'] = $shipping_address['delivery_time'];
				}
			...
			$data['payment_address_1'] = $data['shipping_address_1'] = (isset($this->session->data['payment']['address_1'])) ? $this->session->data['payment']['address_1'] : "";
			...
			$data['send_message'] = (isset($this->session->data['send_message'])) ? $this->session->data['send_message'] : "";
