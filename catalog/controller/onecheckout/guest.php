...
// 				}
				...
				if (($this->session->data['shipping_method']['code'] != "pickup.pickup") && (strlen(utf8_decode($this->request->post['address_1'])) < 3) || (strlen(utf8_decode($this->request->post['address_1'])) > 32)) {
					$json['error']['address_1'] = $this->language->get('error_address_1');
				}

				if (($this->session->data['shipping_type'] == 1) && empty($this->request->post['delivery_time'])) {
					$json['error']['delivery_time'] = $this->language->get('error_delivery_time');
				}
...
// 				}
			...
			if (isset($this->session->data['shipping_type'])) {
				$this->data['shipping_type'] = $this->session->data['shipping_type'];
			} else {
				$this->data['shipping_type'] = $this->session->data['shipping_type'] = 1;
			}
...
/*			if ((strlen(utf8_decode($this->request->post['city'])) < 2) || (strlen(utf8_decode($this->request->post['city'])) > 128)) {
			...
			if (!$json) {
				...
				$this->session->data['guest']['shipping']['delivery_time'] = $this->request->post['delivery_time'];

				$this->session->data['guest']['shipping']['need2call'] = $this->request->post['need2call'];

				if ($this->request->post['need2call'] == 1) {
			...
			if (isset($this->session->data['guest']['shipping']['need2call'])) {
				$this->data['need2call'] = $this->session->data['guest']['shipping']['need2call'];
			} elseif (isset($this->session->data['need2call'])) {
				$this->data['need2call'] = $this->session->data['need2call'];
			} else {
				$this->data['need2call'] = 0;
			}
			...
			if (isset($this->session->data['guest']['shipping']['delivery_time'])) {
				$this->data['delivery_time'] = $this->session->data['guest']['shipping']['delivery_time'];
