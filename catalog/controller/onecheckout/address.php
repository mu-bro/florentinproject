...
class ControllerOneCheckoutAddress extends Controller {
	...
	public function payment() {
		...
		list($this->data['weekends'],$this->data['min_date'],$this->data['allowTimes']) = $this->document->calendarInit();
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			$this->data['address'] = $this->model_onecheckout_checkout->getAddress($this->data['address_id']);
			$this->data['address_1'] = $this->data['address']['address_1'];

			if (isset($this->session->data['payment']['delivery_time'])) {
				$this->data['delivery_time'] = $this->session->data['payment']['delivery_time'];
	...
	public function shipping() {
		...
		$weekends = $this->db->query("SELECT * FROM dates ORDER BY date")->rows;
		$this->data['weekends'] = array();
		foreach ($weekends as $date) {
			$this->data['weekends'][] = $date['date'];
		}
		$cur_hour = date('H');
		$this->data['min_date'] = ($cur_hour < 17) ? date("Y/m/d",strtotime('+1 day')) : date("Y/m/d",strtotime('+2 day'));				
...
// 					}
	...
	public function getinfo() {
		...
		if(isset($this->session->data['payment_address_id']) && $this->session->data['payment_address_id'] && $this->session->data['payment_address_id'] != "undefined"){
		...
		} elseif(isset($this->session->data['guest']['shipping']) && !empty($this->session->data['guest']['shipping'])) {
		...
		if(isset($payment_address)) {
			...
			$json['paymentaddress'] = '
				<div class="sub_header">Покупатель</div>
				<div>
					<label>' . $this->data['entry_firstname'] . '</label>
					<div class="info_data">' . $payment_address['firstname']. '</div>
				</div>
				<br />
				<div>
					<label>' . $this->data['entry_telephone']. '</label>
					<div class="info_data">' .$phone. '</div>
				</div>
				<br />
				<div>
					<label>' . $this->data['entry_email']. '</label>
					<div class="info_data">' .$email. '</div>
				</div><br />';
			if ($this->session->data['shipping_type'] == 1) {
				$json['paymentaddress'] .= '
					<div>
						<label>' . $this->data['text_recipient']. '</label>
						<div class="info_data">Покупателю</div>
					</div>
					<br />				
					<div>
						<label>' . $this->data['entry_address_1']. '</label>
						<div class="info_data">' . $address . '</div>
					</div>
					<br />
					<div>
				...
				$need2call = ($this->session->data['guest']['shipping']['need2call'] == 1) ? "Позвонить" : "Сделать сюрприз";
				$json['paymentaddress'] .= '
					<div class="sub_header">Получатель</div>
					<div>
						<label>' . $this->data['entry_firstname']. '</label>
						<div class="info_data">' .$this->session->data['guest']['shipping']['firstname']. '</div>
					</div>
					<br />
					<div>
						<label>' . $this->data['entry_address_1']. '</label>
						<div class="info_data">' .$this->session->data['guest']['shipping']['address_1']. '</div>
					</div>
					<br />
					<div>
						<label>' . $this->data['entry_delivery_time']. '</label>
					...
					<br />
					<div>
						<label>' . $this->data['text_if_need_to_call']. '</label>
						<div class="info_data">' .$need2call . '</div>
					</div>
					<br />
				';
				if ($this->session->data['guest']['shipping']['need2call'] == 1) {
					$json['paymentaddress'] .= '<div>
							<label>' . $this->data['entry_telephone']. '</label>
							<div class="info_data">' .$this->session->data['guest']['shipping']['telephone'] . '</div>
						</div>
					';
				}
			}
			if (isset($this->session->data['send_message']) && !empty($this->session->data['send_message'])) {
				$json['paymentaddress'] .= '<br /><div>
						<label>' . $this->data['text_send_message']. '</label>
						<div class="info_data">' . ((isset($this->session->data['send_message'])) ? $this->session->data['send_message'] : "") . '</div>
					</div>
				';
			}
		...
		if(isset($shipping_address) && $this->session->data['shipping_type'] == 0) {
			$json['shippingaddress'] = $shipping_address['firstname'] . '<br />' . $shipping_address['address_1'] . ' ' . $shipping_address['address_2']  . '<br />' . $shipping_address['city'] . ' ' . $shipping_address['postcode'] . '<br />' . $shipping_address['zone'] . '<br />' . $shipping_address['country'];
		} else {
		...
		if (isset($this->session->data['shipping_method']['title'])) {
			$json['shippingmethod'] = '<label style="border-bottom:1px #000000 solid;">Доставка</label>' . $this->session->data['shipping_method']['title'];
		} else {
			$json['shippingmethod'] = '';
		}
		
		if (isset($this->session->data['payment_method']['title'])) {
			$json['paymentmethod'] = '<label style="border-bottom:1px #000000 solid;">Оплата</label>' . $this->session->data['payment_method']['title'];
		} else {
			$json['paymentmethod'] = '';
		}
		...
		if (isset($this->session->data['comment']) && !empty($this->session->data['comment'])) {
			...
			$json['paymentmethod'] .= '<br /><br /><label style="border-bottom:1px #000000 solid;">'.$this->language->get('text_checkout_comment').'</label>';
			$json['paymentmethod'] .= '<p style="padding-top:10px;overflow:hidden;text-overflow: ellipsis;">'.$this->session->data['comment'].'</p>';
		}
