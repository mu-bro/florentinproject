...
class ControllerOneCheckoutShipping extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($this->request->get['shipping_type'])) {
            $this->session->data['shipping_type'] = $this->request->get['shipping_type'];
        }


        if (isset($this->request->get['country_id'])) {
            $this->request->get['city'] = $this->config->get('config_zone_id');
            ...
            if (isset($this->request->get['need2call'])) {
                $this->session->data['guest']['shipping']['need2call'] = $this->request->get['need2call'];
                $shipping_address['need2call'] = $this->request->get['need2call'];
            }
            if (isset($this->request->get['delivery_time'])) {
                $this->session->data['guest']['shipping']['delivery_time'] = $this->request->get['delivery_time'];
                $shipping_address['delivery_time'] = $this->request->get['delivery_time'];
            }

            //p($this->request->get,$this->session->data);
        }
        ...
        if (isset($shipping_address)) {
            ...
            if ($this->config->get('config_tax_customer') == 'shipping') {
                $this->session->data['shipping_country_id'] = $shipping_address['country_id'] = $this->config->get("config_country_id");
                $this->session->data['shipping_zone_id'] = $shipping_address['zone_id'] = $this->config->get("config_zone_id");
...
//p($quote_data);
        ...
        if (count($this->cart->getProducts()) > 1) {
            foreach ($this->cart->getProducts() as $product) {
                if ($product['isTwoDaysDelivery']) {
                    $this->data['warningText'] = "Уважаемый покупатель! Букет $product[name] можно заказать не ранее, чем " . date("d.m.Y", strtotime($this->data['min_date'])) . ". Если вы хотите получить остальные букеты этого заказа раньше, то yдалите букет $product[name] из общего списка и оформите его отдельно.";
                }
            }
        }
        ...
        if (isset($this->session->data['payment']['address_1'])) {
            $this->data['address_1'] = $this->session->data['payment']['address_1'];
        } else {
            $this->data['address_1'] = '';
        }
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            $this->session->data['payment']['delivery_time'] = $this->session->data['delivery_time'] = $this->request->post['delivery_time'][$this->request->post['shipping_method']];
            $this->session->data['delivery_interval'] =$this->request->post['delivery_interval'][$this->request->post['shipping_method']];
