...
<div id="content"  class="no_margin"><?php echo $content_top; ?>
  ...
  <h1 class="cart_title"><?php echo $heading_title; ?></h1>
	<div class="checkout_block">
		...
		<?php if (!$logged) { ?>
			<div id="checkout" class="login_block">
				<a id="login-show"><?php echo $text_already_reg; ?></a>
				<div id="login"></div>
			</div>
		<?php } ?>
		...
		<div class="slider-left">
			...
			<?php } ?>
			<div class="onecheckout-content" id="send_message_block">
				<label><?php echo $text_send_message; ?></label>
				<textarea name="send_message" rows="8" style="width:234px;" onblur="saveSendMessage(this.value);"><?php echo $send_message; ?></textarea>
			</div>
		</div>
		...
		<div class="slider-right">
			...
			<div id="beforeconfirmorder">
				<input type="checkbox" name="confirm_agree" value="1" style="float:left;" />
				<div class="agree_text"><?php echo $text_confirm_agree; ?></div>
			</div>
			<div id="confirmorder" style="clear:both"></div>
			<div id="postconfirmorder" style="clear:both;display:none;">
				<a class="button big_b grey" href="javascript: void(0);" onclick="location.reload();" ><span><?php echo $button_change_info; ?></span></a>
			</div>
		</div>
		<div class="clear"></div>
  </div>
  <?php echo $content_bottom; ?></div>
<link rel="stylesheet" type="text/css" href="catalog/view/theme/florentin/stylesheet/onecheckout.css" media="screen" />
<style type="text/css">
<?php if ($this->config->get('onecheckout_style')==3 || !$this->config->get('onecheckout_style')) { ?>
.slider-left{width:244px;float:left;text-align:left;padding-right:35px;}
.slider-mid{width:243px;float:left;margin:0;text-align:left;}
.slider-right{width:240px;float:left;text-align:left;padding-left: 36px;}
...
function saveSendMessage(text) {
	$.ajax({
		url: 'index.php?route=onecheckout/checkout/saveSendMessage',
		type: 'post',
		data: 'message=' + text,
		dataType: 'json'
	});
...
function check_height() {
	height_l = $('.slider-left').height();
	height_m = $('.slider-mid').height();
	height_r = $('.slider-right').height();
	if (height_l < height_r) {
		height_max = height_r;
	//	$('.slider-left').css("min-height",height_r + "px");
	} else {
		height_max = height_l;
	//	$('.slider-right').css("min-height",height_l + "px");
	}
	$('.slider-right').css("min-height",height_l + "px");
	if (height_m < height_max) {
	//	$('.slider-mid').css("min-height",height_max + "px");
	}
}
				...
				$.ajax({
					...
					success: function(json2) {
						...
						if (json2['output']) {
							...
							$('#payment-address .onecheckout-content').prepend(json2['output']);
						}
					}
				});
			}
...
$('#payment-address input[name=\'shipping_address\']').live('change', function() {
	...
	if ($('#payment-address input[name=\'shipping_address\']:checked').attr('value') == 1) {
		...
		shippingmethod(
				$('#payment-address select[name=\'country_id\']').val(),
				$('#payment-address select[name=\'zone_id\']').val(),
				1 ,
				$('#payment-address input[name=\'city\']').val(),
				$('#payment-address input[name=\'postcode\']').val(),
		...
		$('#payment_addr_acc_field').hide();
	} else {
		$('#shipping-address').slideDown('slow');
		shippingmethod(
			$('#shipping-address select[name=\'country_id\']').val(),
			$('#shipping-address select[name=\'zone_id\']').val(),
			1 ,
			$('#shipping-address input[name=\'city\']').val(),
			$('#shipping-address input[name=\'postcode\']').val(),
			$('#payment-address input[name=\'shipping_address\']:checked').attr('value')
			);
...
$('#shipping-method input[name=\'shipping_method\']').live('change', function() {
	...
	shippingMethod = $('#shipping-method input[type=\'radio\']:checked').val();
	$('#shipping-method .shipping_delivery').hide();
	$('#shipping-method .error').remove();
	$('#' + shippingMethod.replace(".","_") + '_sd').show();
				...
				success: function(json) {
					...
					if (json['output']) {
						...
						if($('#guest-cpanle'))
 						...
 						$('#payment-address .onecheckout-content').prepend(json['output']);
						<?php if ($shipping_required) { ?>
							shippingaddress();
						<?php } ?>
					}
...
function shippingmethod(countryid, zoneid, isset, city, postcode, shipping_type) {
	$.ajax({
		url: 'index.php?route=onecheckout/shipping',
		type: 'get',
		data: {countryid: countryid,zoneid: zoneid, isset:isset, city:city, postcode:postcode, shipping_type:shipping_type},
		dataType: 'json',
		beforeSend: function() {
			$('#shipping-method .onecheckout-content').html('<span class="wait">&nbsp;<img src="catalog/view/theme/default/image/loading.gif" alt="" /></span>');
		},		
	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['output']) {
				...
				$('#confirm .onecheckout-content').html(json['output']).show();
				//$('#confirm .onecheckout-content').slideDown('slow',function() {
					check_height();
				//}); ;
...
function createorder(){
  if($('#payment-address input[name=\'account\']:checked').attr('value')){	  
	...
	$.ajax({
		url: 'index.php?route=onecheckout/register',
		type: 'post',
		data: $('#shipping-method input[type=\'text\'], #payment-address input[type=\'text\'], #payment-address input[type=\'password\'], #payment-address input[type=\'checkbox\']:checked, #payment-address input[type=\'radio\']:checked, #payment-address select'),
		dataType: 'json',
		success: function(json) {
			$('.success, .warning').remove();
			$('.error').remove();
	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['error']) {
				...
				if (json['error']['address_1']) {
					$('#shipping-method input[name=\'address_1\'] + br').after('<span id="error_address_1" class="error">' + json['error']['address_1'] + '</span>');
				}

				if (json['error']['delivery_time']) {
					$('#shipping-method input[name=\'delivery_time\'] + div').after('<span id="error_delivery_time" class="error">' + json['error']['delivery_time'] + '</span>');
				}				
	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['output']) {	
				...
				$('#postconfirmorder').show();
				$('#beforeconfirmorder').show();
