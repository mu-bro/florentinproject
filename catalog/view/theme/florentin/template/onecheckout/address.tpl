...
<?php } ?>
	...
	<div>
		...
		<div class="info_data"><?php echo $address['firstname']; ?></div>
	</div>
	<br />
	<div>
		<label><span class="required">*</span> <?php echo $entry_telephone; ?></label>
		<div class="info_data"><?php echo $telephone; ?></div>
	</div>
	<br />
	<div>
		<label><span class="required">*</span> <?php echo $entry_email; ?></label>
		<div class="info_data"><?php echo $email; ?></div>
	</div>
...
$('#payment_addr_field .date').datetimepicker({
	...
	onClose:function() {
		$.ajax({
			url: 'index.php?route=onecheckout/address/paymentDelDate',
			type: 'post',
			data: $('#payment_addr_field input[name=\'delivery_time\']'),
			dataType: 'json'
		});
	},
	format:'d.m.Y H:i'
