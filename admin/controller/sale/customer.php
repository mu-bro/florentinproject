...
$this->data = array_merge( $this->data , $this->language->load('sale/customer'));
...
$this->data = array_merge( $this->data , $this->language->load('sale/customer'));
  	...
  	protected function getList() {
		...
		$this->data['short_stores'] = $this->model_setting_store->getStores();
		...
		if (!empty($customer_info)) {
			...
			foreach ($results as $result) {
				...
				$this->data['ips'][] = array(
					...
					'filter_store_id'  => $this->url->link('sale/customer', 'token=' . $this->session->data['token'] . '&filter_store_id=' . $result['ip'], 'SSL'),
					'ban_ip'     => $ban_ip_total
				);
			}
		}		
...
$this->data = array_merge( $this->data , $this->language->load('error/not_found'));
...
$this->data = array_merge( $this->data , $this->language->load('sale/customer'));
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->user->hasPermission('modify', 'sale/customer')) { 
			...
			$this->data['success'] = $this->data['text_reward_points_added'];
		} else {
			$this->data['success'] = '';
		}
