...
class ModelSaleOrder extends Model {
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'need2call'               => $order_query->row['need2call'],
				...
				'send_message'             => $order_query->row['send_message'],
	...
	public function getOrders($data = array()) {
		...
		if ($this->session->data['user_group_id'] == 11) {
			$sql .= " AND o.store_id = '" . (int)$this->session->data['user_store_id'] . "'";
		} elseif (isset($data['filter_store_id']) && (!empty($data['filter_store_id']) || $data['filter_store_id'] == "0")) {
	...
	public function getTotalOrders($data = array()) {
		...
		if ($this->session->data['user_group_id'] == 11) {
			$sql .= " AND store_id = '" . (int)$this->session->data['user_store_id'] . "'";
		} elseif (isset($data['filter_store_id']) && (!empty($data['filter_store_id']) || $data['filter_store_id'] == "0")) {
	...
	public function getTotalSales() {
		...
		if ($this->user->isManager()) {
			$sql .= " AND store_id = " . $this->user->getStoreId();
		}
		$query = $this->db->query($sql);
		return $query->row['total'];
	}
	...
	public function addOrderHistory($order_id, $data) {
		...
		if ($this->config->get('config_complete_status_id') == $data['order_status_id']) {
			...
			$rewards = $this->db->query("SELECT SUM(reward) as rewards FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'")->row['rewards'];
			$is_reward = $this->db->query("SELECT count(*) as count FROM " . DB_PREFIX . "customer_reward WHERE order_id = '" . (int)$order_id . "'")->row['count'];
			if ($rewards > 0 && !$is_reward) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "customer_reward SET
				customer_id = '" . (int)$order_info['customer_id'] . "',
				description = '" . ($this->language->get('text_order_id') . " " . (int)$order_id) . "',
      	...
      	if ($data['notify']) {
			...
			if ($order_status_query->num_rows) {
				$message .= $language->get('text_order_status') . "<br/>";
				$message .= $order_status_query->row['name'] . "<br/><br/>";
			}
			
			if ($order_info['customer_id']) {
				$message .= $language->get('text_link') . "<br/>";
