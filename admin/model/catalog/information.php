...
class ModelCatalogInformation extends Model {
	public function addInformation($data) {
		...
		foreach ($data['information_description'] as $store_id => $info) {
			foreach ($info as $language_id => $value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "information_description SET information_id = '" . (int)$information_id . "',
				language_id = '" . (int)$language_id . "',
				store_id = '" . (int)$store_id . "',
				title = '" . $this->db->escape($value['title']) . "',
				description = '" . $this->db->escape($value['description']) . "',
				left_description = '" . $this->db->escape($value['left_description']) . "',
				need_left_description = '" . $this->db->escape($value['need_left_description']) . "',
				meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', seo_title = '" . $this->db->escape($value['seo_title']) . "', seo_h1 = '" . $this->db->escape($value['seo_h1']) . "'");
			}
		}
	...
	public function editInformation($information_id, $data) {
		...
		if ($this->user->isManager())  {
			$this->db->query("DELETE FROM " . DB_PREFIX . "information_description WHERE information_id = '" . (int)$information_id . "' AND store_id = " . $this->user->getStoreId());
		} else {
	...
	public function getInformations($data = array()) {
		if ($data) {
			...
			WHERE id.language_id = '" . (int)$this->config->get('config_language_id') . "' AND id.store_id = '". $this->session->data['user_store_id'] ."'";
	...
	public function getInformationDescriptions($information_id) {
		...
		foreach ($query->rows as $result) {
			$information_description_data[$result['store_id']][$result['language_id']] = array(
				'seo_title'        => $result['seo_title'],
				'seo_h1'           => $result['seo_h1'],
				'meta_keyword'     => $result['meta_keyword'],
				'meta_description' => $result['meta_description'],
				'title'       => $result['title'],
				'left_description' => $result['left_description'],
				'need_left_description' => $result['need_left_description'],
